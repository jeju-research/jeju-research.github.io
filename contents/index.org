...
* ☛ TODO 준비해야 할것들 [0/6]
  ...
  - [-] network (무선캐싱,콘텐츠전송, 네트워크 자원 최적화, 에지 컴퓨팅,에지 네트워크 및 분산 시스템)
...
- grid world: A = {up,down,left,right}
 ...
 #+begin_important
 Random Variable은 보통 함수로 설명한다. mapping되는 값이 R(실수)이다. 그런데,  위에서 A, S를 RV라고 하면서, 실수값을 갖지 않는다. 난 이 표현들이 의미적으로는 아무 문제가 없다고 생각한다.
 #+end_important
*** 보상함수(r)
...
#+begin_note
같은 상태에서 같은  action을 취하더라도 보상이 달라진다는 말을 유념해야 할듯하다. 보상은 환경에서 주어진다는 말도 유념해야 할것 같다. 왜냐? 그럴듯하게 들리지만, 구체적인 예를 보기전까지는 피부에 와닿지가 않는다.
#+end_note
